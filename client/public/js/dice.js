...
(function(window, document, parseInt) {
  ...
  var faceHtml = {
    tpl:
      '<div class="dice_circle" style="#{margin};#{border};left:#{left}%;top:#{top}%;width:0;height:0;position:absolute;"></div>',
    position: [
      null,
      [[50, 50]],
      [[50, 30], [50, 70]],
    ...
    forEach.call(faceHtml.position, function(e, i) {
      ...
      if (e) {
        forEach.call(e, function(e_, i_) {
          html.push(
            faceHtml.tpl.replace('#{left}', e_[0]).replace('#{top}', e_[1]),
          );
        });
  ...
  (function(style, string) {
    if (
      ...
      supportPreserve3d('webkit')
    ) {
      prefix = '-webkit-';
      prefixWord = 'webkit';
    } else if (
      typeof style.MozTransform === string &&
      supportPreserve3d('Moz')
    ) {
      prefix = '-moz-';
      prefixWord = 'Moz';
    } else if (typeof style.msTransform === string && supportPreserve3d('ms')) {
      // IE10及以下 都不支持 所需关键属性 `transform-style:preserve-3d` 故暂时不兼容IE
      prefix = '-ms-';
      prefixWord = 'ms';
  ...
  function keepAnimation(dice) {
    ...
    if (prefix !== null) {
      dice.rafId = requestAnimationFrame_(function() {
        // 最开始进来的时候也考虑requestAnimationFrame_ 提高性能
        keepAnimation(dice);
      });
      ...
      css(
        dice.wrapper,
        getRightJsStyleName('transform', prefixWord),
        'rotate(' +
          dice.zAngle +
          'deg) rotateX(' +
          dice.xAngle +
      ...
      dice.curRollIsRotateX
        ? (dice.xAngle =
            dice.xAngle > 360 ? dice.xAngle - 360 : dice.xAngle + 10)
        : (dice.yAngle =
            dice.yAngle > 360 ? dice.yAngle - 360 : dice.yAngle + 10);
    }
  }
  ...
  function endAnimation(dice) {
    ...
    on(
      dice.wrapper,
      getRightJsStyleName('transitionEnd', prefixWord),
      dice._onEndAnimationEnd,
    );
    on(
      dice.wrapper,
      getRightJsStyleName('transitionEnd', prefixWord),
    ...
    css(
      dice.wrapper,
      getRightJsStyleName('transition', prefixWord),
      prefix + 'transform ' + dice.endAnimationTime / 1000 + 's ease-out',
    );
    css(
      dice.wrapper,
      getRightJsStyleName('transform', prefixWord),
  ...
  function Dice(container, formattedConfig) {
    ...
    if (t.hasShadow) {
      ...
      css(
        ...
        prefixWord === 'Moz'
          ? 'boxShadow'
          : getRightJsStyleName('boxShadow', prefixWord),
        '0 0 ' +
          t.edgeLength +
          'px ' +
          (t.edgeLength * 35) / 100 +
    ...
    forEach.call(
      ['wrapper', 'face1', 'face2', 'face3', 'face4', 'face5', 'face6'],
      function(e, i) {
        ...
        if (e === 'wrapper') {
          ...
          if (prefix !== null) {
            css(
              t[e],
              getRightJsStyleName('transformStyle', prefixWord),
              'preserve-3d',
            );
            // css(t[e],getRightJsStyleName('transform',prefixWord),'translateX(0px)')
            css(
              t[e],
              getRightJsStyleName('transformOrigin', prefixWord),
              '50% 50%',
            );
    ...
    if (prefix !== null) {
      ...
      css(
        container,
        getRightJsStyleName('perspectiveOrigin', prefixWord),
        '50% 50%',
      );
    }
    container.appendChild(t.wrapper);
  }
  Dice.prototype.roll = function(valueArr) {
    // todo：提供单次roll的 keepAnimationTime,endAnimationTime,thisRollOnKeepAnimationEnd,thisRollOnEndAnimationEnd 可配，只在这一次roll中生效
    var t = this,
      _valueArr,
      v,
      timeLen,
    ...
    if (prefix !== null) {
      ...
      requestAnimationFrame_(function() {
        keepAnimation(t);
      });
    } else {
      noAnimation(t);
  ...
  var defaultConfig = {
    ...
    hasShadow: true,
    shadowTop: 75,
    keepAnimationTime: 2000,
    endAnimationTime: 800,
  ...
  window.dice = function(container, edgeLength) {
    userConfig = {
      edgeLength: edgeLength,
      radius: 10,
      // hasShadow:false,
      // shadowTop:90,
      // keepAnimationTime: 100000,
      onKeepAnimationEnd: function(result) {
        console.log(
          'onKeepAnimationEnd,result:' +
            result +
            ',time:' +
            new Date().getTime(),
        );
    ...
    if (!userConfig) {
      ...
      for (k in defaultConfig) {
        if (
          userConfig.hasOwnProperty(k) &&
          defaultConfig.hasOwnProperty(k) &&
          callType(defaultConfig[k]) === callType(userConfig[k])
        ) {
